...
equations = [predicate.MetitEquation(ge,var_id=2),
             ...
             predicate.MetitEquation(e4),
             predicate.MetitEquation(e5),
...
system_def = {('s1',) : {'flow' : { x1.diff(t): 1 - sqrt(x1),
                        ...
                        't' : [{'guard':([guard],), 
                               ...
                               {'guard':([MetitPredicate(e7,'=')],),
                                'next_state': ('unsafe',),
                                'updates' : {}}],
                         'inv' : (MetitPredicate(x1-4,'<'),MetitPredicate(x1-6,'>'),MetitPredicate(x2,'<'),MetitPredicate(x2-1,'>'),MetitPredicate(x2-2,'>'), MetitPredicate(x2-2,'=')),
                         'colour':'blue'},
              ('s2',) : {'flow' : { x1.diff(t): 1 - sqrt(x1-x2+1),
                         ...
                         't' : [{'guard':([guard],),
                                 'next_state' : ('s1',),
                                 'updates' : {}}],
                         'inv' : (MetitPredicate(x1-4,'<'),MetitPredicate(x1-6,'>'),MetitPredicate(x2-1,'<'),MetitPredicate(x2-2,'>'),MetitPredicate(x2,'<'),MetitPredicate(x2,'='),MetitPredicate(x2-0.5,'<'),MetitPredicate(x2-0.5,'=')),
                         'colour':'green'}}
