...
system_def = {('on',) : {'flow' : {x.diff(t): -x(t)+100,
                                   t.diff(t): 1},
                           't' : [{'guard': [[g_pred_80gt],[g_pred_80eq]], 'next_state' : ('off',), 'updates' : ()}],
                           'inv' : (g_pred_82eq,g_pred_82gt)},  
              ('off',) : {'flow': {x.diff(t): -x(t),
                                   t.diff(t): 1},
                            't' : [{'guard': [[g_pred_70lt],[g_pred_70eq]], 'next_state' : ('on',), 'updates' : ()}],
                            'inv' : (g_pred_68eq,g_pred_68lt)}}
 
equations = [predicate.MetitEquation(x(t)-70,var_id=1),
             predicate.MetitEquation(x(t)-80,var_id=1),
             predicate.MetitEquation(x(t),var_id=1),
             predicate.MetitEquation(x(t)-82,var_id=1),
             predicate.MetitEquation(x(t)-68,var_id=1),
             predicate.MetitEquation(t),
             predicate.MetitEquation(t-10)]
